...
from qwen3d.global_vars import (
    ...
    SCANNET_INSTANCE_CLASS_NAMES_200,
    ARKIT_CLASS_LABELS,
...
class Scannet_Evaluator:
    ...
    def evaluate_matches(self, matches):
        ...
        has_gt_full = np.zeros(
            (len(dist_threshes), len(overlaps), len(self.CLASS_LABELS)), bool
        )
        has_pred_full = np.zeros(
            (len(dist_threshes), len(overlaps), len(self.CLASS_LABELS)), bool
        )
        y_score_full = np.empty(
            (len(dist_threshes), len(overlaps), len(self.CLASS_LABELS)), object
        )
        y_true_full = np.empty(
            (len(dist_threshes), len(overlaps), len(self.CLASS_LABELS)), object
        )
        hard_false_negatives_full = np.zeros(
            (len(dist_threshes), len(overlaps), len(self.CLASS_LABELS)), int
        )
    ...
    def compute_ap(
        ...
        ap = np.zeros(
            (len(dist_threshes), len(self.CLASS_LABELS), len(overlaps)), float
        )
    ...
    def compute_averages(self, aps):
        ...
        if False:
            # print all APs at all thresholds
            print("APs at all thresholds")
            for oi, overlap_th in enumerate(opt["overlaps"]):
                print("overlap_th: %f" % overlap_th)
                oth = np.where(np.isclose(opt["overlaps"], overlap_th))
                print(np.nanmean(aps[d_inf, :, oth]))
        ...
        if self.EVALUATE_SUBSET is not None:
            for name in self.EVALUATE_SUBSET:
                ...
                remove_ids = None
                if "scannet200" in self.dataset_name:
                    remove_ids = [199, 200]
                ...
                avg_dict[f"{name}_ap"] = np.nanmean(
                    aps[d_inf, :, oAllBut25][..., class_ids]
                )
                avg_dict[f"{name}_ap_50%"] = np.nanmean(
                    aps[d_inf, :, o50][..., class_ids]
                )
                avg_dict[f"{name}_ap_25%"] = np.nanmean(
                    aps[d_inf, :, o25][..., class_ids]
                )
        return avg_dict
    ...
    def assign_instances_for_scan(self, pred: dict, gt: dict):
        ...
        bool_void = np.logical_not(
            np.in1d(gt_ids // self.multiplier, self.VALID_CLASS_IDS)
        )
        ...
        for uuid in pred_info:
            ...
            assert len(pred_mask) == len(
                gt_ids
            ), f"pred_mask: {len(pred_mask)}, gt_ids: {len(gt_ids)}"
            # convert to binary
