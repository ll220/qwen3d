...
from qwen3d.global_vars import SCANNET_COLOR_MAP_20
...
def sample_2D_indices(new_h, new_w, multiview_p2v, num_images):
    pixel_positions = []
    for b in range(len(multiview_p2v)):
    # Create a grid of (h, w) indices
        h_indices = torch.arange(new_h, device=multiview_p2v[0].device).view(-1, 1).expand(new_h, new_w)
        w_indices = torch.arange(new_w, device=multiview_p2v[0].device).view(1, -1).expand(new_h, new_w)
        pixel_indices = torch.stack([h_indices, w_indices], dim=-1)  # shape: (new_h, new_w, 2)
        pixel_indices = pixel_indices.unsqueeze(0).expand(num_images[b], new_h, new_w, 2).reshape(-1, 2)

        sampled_pixels = torch_scatter.scatter_min(pixel_indices, multiview_p2v[b], dim=0)[0]
        pixel_positions.append(sampled_pixels)
    return pixel_positions


    # reshape it to (bs, , 2)
    # do torch.scatter_min on multiview_p2v
    # sample the indices on outputs of torch.scatter_min
    # return the indices
    return None
...
def plot_3d_new(
    ...
    v = vis.Visualizer()
    v.add_points(
        "RGB", pcd, colors=color * 255, alpha=0.8, visible=False, point_size=point_size
    )
...
def plot_3d_old(
    ...
    if gt_labels is not None and gt_masks is not None:
        ...
        if gt_masks is not None:
            for i, mask in enumerate(gt_masks):
                ...
                mask = mask.nonzero()[0]
                color_masks[mask] = (
                    np.array(SCANNET_COLOR_MAP_20[label.item() + 1]) / 255.0
                )
    ...
    log_dict = {
        "pred: top, gt: bottom ; rgb/semantics/instance": [
            wandb.Image(fig1),
            wandb.Image(fig2),
            wandb.Image(fig3),
        ]
...
def convert_3d_to_2d_dict_format(format_B):
    ...
    for cls_name, cls_data in format_B["classes"].items():
        ...
        segm_per_class[f"AP-{cls_name}"] = (
            np.nan if np.isnan(cls_data["ap"]) else cls_data["ap"]
        )
...
def convert_3d_to_2d_dict_format_semantic(data_dict):
    ...
    return OrderedDict(
        [("3d_semantic_per_class", segm_per_class), ("3d_semantic", segm)]
    )
