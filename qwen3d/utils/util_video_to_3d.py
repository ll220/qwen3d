...
def convert_video_instances_to_3d(
    ...
    w_pad,
    device,
    convert_point_semantic_instance=False,
    skip_classes=None,
    multiplier=1000,
    ...
    all_instances = list(
        itertools.chain.from_iterable(
            [instances.instance_ids for instances in instances_all]
        )
    )
    ...
    if skip_classes is not None:
        unique_instances = unique_instances[
            ~torch.tensor(skip_classes)
            .unsqueeze(1)
            .eq(unique_instances // multiplier + 1)
            .any(0)
        ]
    ...
    if device is None:
        ...
        gt_classes_per_video = torch.zeros(
            num_instances, dtype=torch.int64, device=device
        )
    ...
    for f_i, targets_per_frame in enumerate(instances_all):
        ...
        for idx, instance_id in enumerate(gt_instance_ids):
            ...
            if convert_point_semantic_instance:
                new_instance_id = (
                    gt_cls[idx] + 1
                ) * multiplier + instance_id % multiplier
                point_semantic_instance_label_per_video[f_i, :h, :w][
                    targets_per_frame.gt_masks[idx]
                ] = new_instance_id

    target_dict = {"labels": gt_classes_per_video, "masks": gt_masks_per_video}

    if convert_point_semantic_instance:
        target_dict[
            "point_semantic_instance_label"
        ] = point_semantic_instance_label_per_video
