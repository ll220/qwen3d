...
def last_indices_per_row(tensor, token_id):
    # boolean mask where token matches
    mask = (tensor == token_id)
    
    # if we reverse along seq dim, the first True gives us the last occurrence
    reversed_idx = mask.flip(dims=[1]).float().argmax(dim=1)
    # convert back to original indices
    last_idx = tensor.size(1) - 1 - reversed_idx
    return last_idx
